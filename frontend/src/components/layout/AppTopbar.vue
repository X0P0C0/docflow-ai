<template>
  <div class="topbar-stack">
    <header class="topbar">
      ...
      >
        <span v-if="isSidebarOpen">&times;</span>
        <span v-else>&#9776;</span>
      </button>
      ...
      <button
        ...
        type="button"
        @click="navigateTo('/knowledge/articles')"
      >
        搜索知识文章、工单、标签或成员...
      </button>
...
const modeChipClass = computed(() =>
  demoMode.value ? "chip-orange" : "chip-green"
);
const roleText = computed(() => authState.user?.roles?.join(" / ") || "访客");
const bannerClass = computed(() =>
  demoMode.value ? "runtime-banner-demo" : "runtime-banner-live"
);
