        ...
        <div class="ai-header workspace-header">
          <div class="ai-header__main workspace-header-main">
            <div class="ai-header__title workspace-title">
              <span class="page-eyebrow">AI 工作区</span>
              <h2>AI 协作工作区</h2>
              <p>
            ...
            <div class="ai-header__actions workspace-actions">
              <RouterLink class="ghost-button ai-link-button" to="/tickets">查看队列</RouterLink>
            </div>
          </div>
      ...
      <section class="ai-main-grid">
        <article class="panel">
          ...
          <div v-if="aiLoading || aiErrorMessage" class="state-box ai-state-box" :class="{ 'state-warning': aiLoading || usedFallbackData }">
            <strong>{{ aiLoading ? '正在加载 AI 工作区' : '使用离线 AI 工作区预览' }}</strong>
            <p>{{ aiLoading ? '正在从后端获取回复建议和知识推荐.' : aiErrorMessage }}</p>
            <p v-if="aiErrorTraceId" class="ai-state-trace">追踪ID: {{ aiErrorTraceId }}</p>
          </div>
          ...
          <div class="ai-highlight-card workspace-highlight-card">
            ...
            <div class="ai-highlight-meta workspace-highlight-meta">
              <span>场景: {{ replySuggestion.scene }}</span>
              <span>置信度: {{ replySuggestion.confidence }}</span>
            </div>
            <div class="ai-inline-actions">
              ...
              >
                {{ draftLoading && selectedTicketId === replySuggestion.ticketId ? '加载草稿中...' : 'Open draft' }}
              </button>
              <RouterLink
        ...
        <article class="panel">
          ...
          <div v-if="draftLoading || draftErrorMessage" class="state-box ai-state-box" :class="{ 'state-warning': draftLoading || draftUsedFallback }">
            <strong>{{ draftLoading ? '正在加载回复草稿' : '使用离线草稿预览' }}</strong>
            <p>{{ draftLoading ? '正在为选中工单生成详细操作草稿。' : draftErrorMessage }}</p>
            <p v-if="draftErrorTraceId" class="ai-state-trace">追踪ID: {{ draftErrorTraceId }}</p>
          </div>
          ...
          <div class="ai-draft-notes">
            ...
            <div v-if="selectedTicketId" class="ai-inline-actions">
              ...
              >
                {{ knowledgeDraftSubmitting ? '创建草稿中...' : '创建知识草稿' }}
              </button>
              <button
                class="ghost-button ai-link-button"
                type="button"
            ...
            >
              <strong>{{ knowledgeDraftTraceId ? '知识草稿需要处理' : 'Knowledge draft action updated' }}</strong>
              <p>{{ knowledgeDraftMessage }}</p>
              <p v-if="knowledgeDraftTraceId" class="ai-state-trace">追踪ID: {{ knowledgeDraftTraceId }}</p>
            </div>
          </div>
        </article>
      </section>
      ...
      <section class="ai-secondary-grid">
        <article class="panel">
          <div class="panel-head">
            ...
            <RouterLink class="ghost-button ai-link-button" to="/knowledge/articles">打开知识库</RouterLink>
          </div>
              ...
              <div class="mini-list">
                ...
                >
                  ...
                  <div class="ai-followup-actions">
                    ...
                    >
                      {{ isFollowupClaimedByAnotherUser(item) ? '加载已认领草稿' : '加载草稿' }}
                    </button>
                    <button
      ...
      >
        <section class="ai-modal-card">
          ...
          <div class="mini-list">
            <div class="mini-item">
              <span>当前负责人</span>
              <strong>{{ activeDraft.adoptedByName || '未知操作员' }}</strong>
            </div>
            <div class="mini-item">
              <span>认领时间</span>
            ...
            <div class="mini-item">
              <span>下一步操作</span>
              <strong>你将替换当前共享认领为 {{ currentUserName }}.</strong>
            </div>
          </div>
          <p class="ai-action-hint">
            {{ isClaimStale(activeDraft.claimFreshness, activeDraft.lastActivityAt, activeDraft.adoptedAt)
              ? '此认领已超出活跃窗口, so it may no longer reflect active ownership.'
              : '仅当你需要立即从当前负责人接管此回复时继续。' }}
          </p>
          <div class="ai-inline-actions ai-modal-actions">
            ...
            >
              {{ replyActionSubmitting ? '正在接管认领...' : '确认接管' }}
            </button>
          </div>
        </section>
      </div>
...
const followupGroups = computed(() => {
  ...
  for (const item of followups.value) {
    if (item.adopted || (item.ticketId && adoptedTicketIds.value.includes(item.ticketId))) {
      const claimedItem = {
        ...item,
        chip: isClaimStale(item.claimFreshness, item.lastActivityAt, item.adoptedAt) ? '认领可能已过期' : 'Claimed in workspace',
  ...
  return [
    ...
    },
    {
      key: 'claimed-by-me',
      title: 'Claimed By Me',
      description: 'Drafts this operator is actively carrying in the shared AI 工作区.',
      chipClass: 'chip-blue',
      items: claimedByMe,
    },
    {
...
const generatedAtText = computed(() => {
  if (!aiWorkspace.value.generatedAt) {
    return usedFallbackData.value || false
...
function isOwnedByAnotherUser(adoptedByUserId?: number | null, adoptedByName?: string) {
  if (!adoptedByName || adoptedByName === '当前浏览器') {
    return false
  }
  ...
  aiWorkspace.value = {
    ...
    primarySuggestion: aiWorkspace.value.primarySuggestion.ticketId === ticketId
      ? {
          ...
          claimFreshness: adopted ? (claimFreshness || '活跃') : '',
        }
      : aiWorkspace.value.primarySuggestion,
...
async function markReplyDraftAsAdopted(skipTakeoverConfirmation = false) {
  ...
  try {
    ...
    activeDraft.value = {
      ...
      claimFreshness: response.claimFreshness || '活跃',
    }
    ...
    replyActionMessage.value = takingOverClaim && previousAdoptedByName && response.adoptedByName
      ? `Reassigned this shared AI 工作区 claim from ${previousAdoptedByName} to ${response.adoptedByName}.`
      : response.adoptedByName
      ? `已将此回复草稿标记为 AI 工作区已采纳 for ${response.adoptedByName}.`
      : '已将此回复草稿标记为 AI 工作区已采纳 so the team can see it has already been handled.'
  } catch (error) {
    ...
    activeDraft.value = {
      ...
      adopted: true,
      adoptedByUserId: null,
      adoptedByName: takingOverClaim ? `${currentUserName.value} (local)` : '当前浏览器',
    ...
    updateWorkspaceAdoptionState(
      selectedTicketId.value,
      true,
      takingOverClaim ? `${currentUserName.value} (local)` : '当前浏览器',
      localAdoptedAt,
      null,
      localAdoptedAt,
      '活跃',
    ...
    replyActionMessage.value = takingOverClaim
      ? '共享采纳状态暂不可用, 当前浏览器已在本地接管认领 only.'
      : '共享采纳状态暂不可用, 此草稿已在本地标记为已采纳 in this browser instead.'
    console.error(error)
  } finally {
    replyActionSubmitting.value = false
  }
...
async function unmarkReplyDraftAsAdopted() {
  ...
  try {
    ...
    replyActionMessage.value = '已从 AI 工作区采纳队列中移除该回复草稿。'
  } catch (error) {
    ...
    replyActionMessage.value = '共享采纳状态暂不可用, so the local adopted marker was cleared instead.'
    console.error(error)
  } finally {
    replyActionSubmitting.value = false
  }
...
async function loadAiWorkspace() {
  ...
  try {
    ...
    const result = resolveListLoadFailure(error, {
      networkFallbackMessage: 'AI 工作区暂不可用。 The page has fallen back to the local preview.',
      defaultMessage: '加载 AI 工作区失败。 The page has kept the local preview so the team can continue reviewing the surface.',
    })
...
async function selectTicket(ticketId?: number | null) {
  ...
  try {
    ...
    const result = resolveListLoadFailure(error, {
      networkFallbackMessage: '草稿生成服务暂不可用。 The page has kept the preview draft.',
      defaultMessage: '加载回复草稿失败。 The page has kept the preview draft so the operator workflow still renders.',
    })
...
async function createKnowledgeDraftFromAiCenter() {
  ...
  try {
    ...
    knowledgeDraftMessage.value = '已从选中工单创建知识草稿。 Opening the editor now.'
    ...
    try {
      ...
      knowledgeDraftMessage.value = '知识草稿服务暂不可用，已准备本地草稿。'
      await router.push('/knowledge/articles/create?from=ticket')
    } catch (fallbackError) {
      const result = resolveListLoadFailure(fallbackError, {
        networkFallbackMessage: '知识草稿路径暂不可用，本地备选草稿也无法准备。',
        defaultMessage: '无法从选中工单准备知识草稿。',
      })
      knowledgeDraftMessage.value = result.message
      knowledgeDraftTraceId.value = result.traceId
